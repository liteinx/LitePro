// Watch Together (w2g) Showcase Script with Multi-language Support
...
(function() {
    ...
    function applyLanguage(lang) {
        ...
        // Update page title
        if (t.title) {
            document.title = `${t.title} - Sync Video Watching with Friends`;
        }

        // Update meta description
        const metaDesc = document.querySelector('meta[name="description"]');
        if (metaDesc && t.tagline) {
            metaDesc.setAttribute('content', `${t.tagline} - Watch videos with friends and family in real-time.`);
        }
    }
    ...
    async function loadAPKConfig() {
        try {
            ...
            if (code && config.apps && config.apps[code]) {
                ...
                state.apkName = 'Watch Together'; // Always show default name
            ...
            state.currentAPK = 'w2g_default.apk';
            state.apkName = 'Watch Together';
    ...
    window.addEventListener('load', () => {
        ...
        console.log(`Watch Together showcase loaded in ${loadTime}ms`);
    });
